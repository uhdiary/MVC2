...
<div id = "msg1" style = "display: none"><!-- 화면상으로는 안보임 -->
 <h3>내용을 입력하여 주세요!!</h3></div>
	...
	<form name="writeform" action="/hotel/brwrite?boardno=<%=board.getBoardNo()%>" method="post" onsubmit="return check()">
		<input type="hidden" name="seq" value="<%=board.getBoardReplySeq()%>">
		<input type="hidden" name="boardWrite" value="<%=loginUser.getUser_id()%>">
		<input type="hidden" name="boardTitle" value="<%=board.getBoardTitle()%>">
		<input type="hidden" name="setp" value="<%=board.getBoardReplyLev()%>">
			
		<table class="type09">
			<thead>
				<tr>
					<th style="font-size:25px" scope="cols" colspan="2">답글 쓰기</th>
				</tr>
			</thead>
			<tbody>
				<tr>
					...
					<td><input type="text" id="boardTitle" name="boardTitle"
						size="80px" style="height: 37px; text-align: center;"
						value="<%=board.getBoardTitle()%>" readonly></td>
				</tr>
				<tr>
					<th scope="row">작성자</th>
		...
		<br> <input id="button1" type="submit" value="답변"> &nbsp;
		<tr>
